...
# ============================================
# MODULES 5-10 : All remaining modules
# ============================================

# Helper: make a full module

def make_module(name, num, dir_name, class_prefix, table, module_code, sql_code, class_code, index_code, card_code, test_code, label):
    base = f"{BASE}/{dir_name}"
    write(f"{base}/core/modules/mod{class_prefix}.class.php", module_code)
    write(f"{base}/sql/llx_{table}.sql", sql_code)
    write(f"{base}/class/{table}.class.php", class_code)
    write(f"{base}/index.php", index_code)
    write(f"{base}/card.php", card_code)
    write(f"{base}/test/phpunit/{class_prefix}Test.php", test_code)

# --- 5. immovente ---
make_module("immovente", 700004, "dolibarr-agence-immovente", "Immovente", "immo_mandat_vente",
'''<?php
...
print("Done generating all modules")
